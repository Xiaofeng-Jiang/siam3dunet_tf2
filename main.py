...
config["training_modalities"] = [
    "t1"]  # config["all_modalities"]  # change this if you want to only use some of the modalities
...
def main(overwrite=False):
    # convert input images into a hdf5 file
    if overwrite or not (os.path.exists(config["data_file0"]) and os.path.exists(config["data_file1"])):
        ...
        if not os.path.exists(config["data_file0"]):
            write_data_to_file(training_files0, config["data_file0"], image_shape=config["image_shape"],
                               subject_ids=subject_ids0)
        if not os.path.exists(config["data_file1"]):
            write_data_to_file(training_files1, config["data_file1"], image_shape=config["image_shape"],
    ...
    if not overwrite and os.path.exists(config["model_file"]):
        ...
        model = siam3dunet_model(input_shape=config["input_shape"], n_labels=config["n_labels"],
